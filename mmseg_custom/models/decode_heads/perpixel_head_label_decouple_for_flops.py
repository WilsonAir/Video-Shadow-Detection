...
class PerPixelDecoupleHead(BaseDecodeHead):
    ...
    def loss(self, all_cls_scores, all_mask_preds, gt_labels_list,
        ...
        # predictions = all_mask_preds[0].float()  # https://github.com/pytorch/pytorch/issues/48163
        # predictions = F.interpolate(
        #     predictions, size=(gt_masks_list[0].shape[-2], gt_masks_list[0].shape[-1]), mode="bilinear",
        #     align_corners=False
        # )
        gt_new = []
        ...
        #     predictions, gt, reduction="mean"  # , ignore_index=self.ignore_value
        # )
        # full_image = torch.ones(size=gt_masks_list[0].shape, device=gt_masks_list[0].device,
        #                         dtype=torch.float)
        # gt_float = torch.tensor(gt, dtype=torch.float, device=gt.device)
        # loss_structure_full = F.mse_loss(predictions[:, 0, :, :] + predictions[:, 1, :, :], target=full_image)
        # loss_structure_inv_s = F.mse_loss(full_image - predictions[:, 0, :, :], target=gt_float)
        # loss_structure_inv_ns = F.mse_loss(full_image - predictions[:, 1, :, :], target=full_image - gt_float)
        ...
        # cv.imshow('test_0', body_fuse[0][0].astype(np.uint8))
        # cv.imshow('test_1', body_fuse[1][0].astype(np.uint8))
        # cv.imshow('detail_0', detail_o[0].astype(np.uint8))
        # cv.imshow('detail_1', detail_o[1].astype(np.uint8))
        # cv.waitKey(0)
        ...
        # loss_dict['loss_structure_full'] = loss_structure_full
        # loss_dict['loss_structure_inv_s'] = loss_structure_inv_s
        # loss_dict['loss_structure_inv_ns'] = loss_structure_inv_ns
    ...
    def forward_test(self, inputs, img_metas=None, test_cfg=None):
        ...
        """
        all_cls_scores, all_mask_preds = self(inputs)
